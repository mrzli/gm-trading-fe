...
import { positionsBox, positionsLine } from '../helpers';
...
class UserPriceLineDataBase {
  ...
  protected _data: UserPriceLinesData;

  public constructor(data: UserPriceLinesData) {
    this._data = data;
  }

  public update(
    data: UserPriceLinesData,
    ...
    this._y = series.priceToCoordinate(this._data.price) ?? -10_000;
  }
}
...
class UserPriceLinesPaneRenderer implements ISeriesPrimitivePaneRenderer {
  private _data: UserPriceLinesRendererData;

  public constructor(data: UserPriceLinesRendererData) {
    this._data = data;
  }
  ...
  public draw(target: CanvasRenderingTarget2D): void {
    ...
    target.useBitmapCoordinateSpace((scope) => {
      ...
      const roundedArray = [5, 0, 0, 5].map(
        (i) => i * scope.horizontalPixelRatio,
      );
      ctx.beginPath();
      ctx.roundRect(
        xPos.position,
...
class UserPriceLinesLabelButton extends PluginBase {
  private _paneViews: UserPriceLinesPaneView[];
  private _data: UserPriceLinesData = {
    visible: false,
    hovered: false,
    timeScaleWidth: 0,
  ...
  private _source: UserPriceLines;

  public constructor(source: UserPriceLines) {
    super();
  ...
  public updateAllViews(): void {
    for (const pw of this._paneViews) pw.update(this._data, this.series);
  }

  public priceAxisViews(): never[] {
    return [];
  }

  public paneViews(): readonly UserPriceLinesPaneView[] {
    return this._paneViews;
  }

  public showAddLabel(price: number, hovered: boolean): void {
...
export class UserPriceLines {
  ...
  public currentLineColor(): string {
  ...
  private _setCrosshairMode(): void {
    if (!this._chart) {
      throw new Error(
  ...
  private _getMousePrice(param: MouseEventParams): BarPrice | null {
    if (!param.point || !this._series) return null;
    const price = this._series.coordinateToPrice(param.point.y);
    return price;
  }

  private _distanceFromRightScale(param: MouseEventParams): number | null {
